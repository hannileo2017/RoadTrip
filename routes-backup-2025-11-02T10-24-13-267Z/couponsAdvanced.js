...
router.get('/', async (req, res) => {
    try {
        ...
        const countResult = await sql.query(`SELECT COUNT(*) AS "TotalCount" FROM "couponsadvanced" ${sql.raw(where)}`);
...
router.get('/:id', async (req, res) => {
    try {
        ...
        const result = await sql.query(`SELECT * FROM "couponsadvanced" WHERE "CouponID" = ${id}`);
...
router.get('/validate/:code', async (req, res) => {
    try {
        ...
        const result = await sql.query(`SELECT * FROM "couponsadvanced" WHERE "Code" = ${code}`);
        if (!result.length) return sendResponse(res, false, 'Coupon not found', null, 404);
...
router.post('/', isAdmin, async (req, res) => {
    try {
        ...
        const exists = await sql.query(`SELECT "CouponID" FROM "couponsadvanced" WHERE "Code" = ${Code.trim()}`);
...
router.put('/:id', isAdmin, async (req, res) => {
    try {
        ...
        const couponCheck = await sql.query(`SELECT * FROM "couponsadvanced" WHERE "CouponID" = ${id}`);
        if (!couponCheck.length) return sendResponse(res, false, 'Coupon not found', null, 404);

        if (fields.includes('Code')) {
            const dup = await sql.query(`SELECT "CouponID" FROM "couponsadvanced" WHERE "Code" = ${updates.Code} AND "CouponID" <> ${id}`);
            if (dup.length) return sendResponse(res, false, 'Another coupon with this code exists', null, 409);
        }

        const setClauses = fields.map((f, i) => `"${f}" = $${i+1}`).join(', ');
        const values = fields.map(f => updates[f]);
        await sql.query(`UPDATE "CouponsAdvanced" SET ${sql.raw(setClauses)}, "UpdatedAt" = NOW() WHERE "CouponID" = ${id}`).bind(values);
...
router.delete('/:id', isAdmin, async (req, res) => {
    try {
        ...
        const exists = await sql.query(`SELECT "CouponID" FROM "couponsadvanced" WHERE "CouponID" = ${id}`);
        if (!exists.length) return sendResponse(res, false, 'Coupon not found', null, 404);

        await sql.query(`DELETE FROM "couponsadvanced" WHERE "CouponID" = ${id}`);
