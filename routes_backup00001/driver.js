...
async function generateDriverID() {
  const row = await sql.query(`SELECT coalesce(max(cast(substring("DriverID", 5) AS INTEGER)), 0) AS maxnum FROM "Drivers";`, [/* add params here */]);
...
router.get('/', async (req, res) => {
  try {
    const rows = await sql.query(`SELECT * FROM "Drivers" ORDER BY "CreatedAt" DESC LIMIT 20;`, [/* add params here */]);
...
router.put('/:driverid', async (req, res) => {
  try {
    ...
    const existingRows = await sql.query(`SELECT * FROM "Drivers" WHERE "DriverID" = $1 LIMIT 1;`, [/* add params here */]);
...
router.delete('/:driverid', async (req, res) => {
  try {
    ...
    const deleted = await sql.query(`DELETE FROM "Drivers" WHERE "DriverID" = $1 RETURNING *;`, [/* add params here */]);
